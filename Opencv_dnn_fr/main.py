if __name__ == "__main__":  
    ...
    path = 'Opencv_dnn_fr/images/known/'#Path of known images to compare from
    ...
    with open("Opencv_dnn_fr/data/encodings.json", 'r+') as f:
        data = json.load(f)

    encodeListKnown = list(data.values())

    #Load model & config file
    modelFile = "Opencv_dnn_fr/model/res10_300x300_ssd_iter_140000.caffemodel"
    configFile = "Opencv_dnn_fr/model/deploy.prototxt.txt"
    ...
    while(True):
        ...
        h,w,_ = imgS.shape
        cv2.imwrite("Opencv_dnn_fr/1.jpg",imgS)
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame, facesCurFrame):
            ...
            if faceDis[matchIndex]<tolerance:
                if matches[matchIndex]:
                    ...
                    cv2.imwrite("Opencv_dnn_fr/output.jpg", imgS)    
                    # markAttendance(name)
                else:
        ...
        cv2.imwrite("Opencv_dnn_fr/2.jpg", imgS)
