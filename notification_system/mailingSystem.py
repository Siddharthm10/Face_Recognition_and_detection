    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                './notification_system/creds/credentials.json', SCOPES)
...
if __name__ == '__main__':
    ''' 
    Description = Sends an email using the GOOGLE API to the user email-ID using regarding unknown visitor.

    '''
    service = google_verification()
    send_mail(service)
